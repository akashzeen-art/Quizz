        ...
        {rows.map((row, i) => {
          ...
          return (
            <motion.li key={row.userId}
              ...
              className={`flex items-center gap-3 px-3 py-3 sm:px-4 ${isMe ? 'bg-violet-50/90 ring-1 ring-inset ring-violet-200/80' : ''}`}>
              <span className="w-8 shrink-0 text-center text-xs font-bold tabular-nums text-slate-400">#{row.rank}</span>
              <div className="flex min-w-0 flex-1 items-center gap-3">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-bold text-white ${isMe ? 'bg-violet-600' : 'bg-gradient-to-br from-slate-500 to-slate-700'}`}>
                  {initial(row.displayName)}
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-900">
                    {row.displayName}
                    {isMe && <span className="ml-2 rounded-md bg-violet-600/10 px-1.5 py-0.5 text-[10px] font-bold text-violet-700">You</span>}
                  </p>
              ...
              <span className="shrink-0 text-sm font-black tabular-nums text-violet-600">{main.toLocaleString()}</span>
            </motion.li>
          )
        })}
      </ul>
